require 'test_helper'

class DropsControllerTest < ActionController::TestCase
  setup do
    @drop = drops(:one)
  end

  test "should get index" do
    get :index
    assert_response :success
    assert_not_nil assigns(:drops)
  end

  test "should get new" do
    get :new
    assert_response :success
  end

  test "should create drop" do
    assert_difference('Drop.count') do
      post :create, drop: { comment: @drop.comment, getup_at: @drop.getup_at, kcal: @drop.kcal, km: @drop.km, pp: @drop.pp, sleep_at: @drop.sleep_at, step: @drop.step, today: @drop.today, xx: @drop.xx }
    end

    assert_redirected_to drop_path(assigns(:drop))
  end

  test "should show drop" do
    get :show, id: @drop
    assert_response :success
  end

  test "should get edit" do
    get :edit, id: @drop
    assert_response :success
  end

  test "should update drop" do
    patch :update, id: @drop, drop: { comment: @drop.comment, getup_at: @drop.getup_at, kcal: @drop.kcal, km: @drop.km, pp: @drop.pp, sleep_at: @drop.sleep_at, step: @drop.step, today: @drop.today, xx: @drop.xx }
    assert_redirected_to drop_path(assigns(:drop))
  end

  test "should destroy drop" do
    assert_difference('Drop.count', -1) do
      delete :destroy, id: @drop
    end

    assert_redirected_to drops_path
  end
end
